...
object ComponentRegistry {
    ...
    fun getRegistryStats(): RegistryStats {
        return RegistryStats(
            ...
            mostUsedTags =
                tagIndex.entries.sortedByDescending { it.value.size }.take(10)
    ...
    fun suggestComponents(
        ...
        val similarByTags =
            baseComponent.tags.flatMap { tag ->
                searchByTags(listOf(tag))
            }.filter { it.id != baseComponent.id }

        // Find components in same category
        val similarByCategory =
            getComponentsByCategory(baseComponent.category)
                .filter { it.id != baseComponent.id }

        // Combine and rank
        val suggestions =
            (similarByTags + similarByCategory)
                .groupBy { it.id }
